...
if(isset($_GET['submit-search'])){
 ...
 if ( ! empty( $_GET['location'] ) || ! empty( $_GET['agencytype'] ) ) {
	 ...
	 if ( ! empty( $_GET['agencytype'] ) ){
		 ...
		 $tax_query[] = array(
		 	'taxonomy' => 'agencytype',
			'field' => 'slug',
			'terms' => sanitize_text_field( $_GET['agencytype'] ),
		 );
...
if ( $loop->have_posts() ) {
        _e("<h3 style='font-weight:bold;color:#717171;'>Search Results for: ".  $_GET['q'] ."</h3>");
...
<?php
} //end if(isset($_GET['submit-search']))
?>
